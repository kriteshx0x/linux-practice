...
# Day 6 notes : journalctl,aliases,zshrc,ps1

## what I learned
- journalctl queries logs from systemd journal (binary, not plain text)
- Filter logs by service (-u ssh), time (--since), priority (-p err)
- Aliases are shell shortcuts NOT for anything needing arguments
- .zshrc runs on every interactive shell; .profile runs on login
- PS1 controls your prompt: \u = user, \h = host, \w = dir
- Kitty terminal replaces tmux for local splits/tabs

## needs more practice
- Combining journalctl flags (time + service + priority together)
- Writing bash functions instead of aliases for complex commands
- PS1 color codes from memory (\033[01;32m etc.)
- Difference between .zshrc, .profile
